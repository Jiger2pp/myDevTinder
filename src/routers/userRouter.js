...
userRouter.get("/user/connections", userAuth, async (req, res) => {
    try{
        const loggedInUser = req.user;
        const connections = await ConnectionRequest.find({ $or: [{"fromUserId" : loggedInUser._id}, {"toUserId" :  loggedInUser._id}], $and: [{status: "accepted"}]}).populate("fromUserId", ["firstName", "lastName", "about", "pictureUrl"]).populate("toUserId", ["firstName", "lastName", "about", "pictureUrl"]);
...
userRouter.get("/user/feed", userAuth, async(req, res) => {
    ...
    try{
        ...
        const myFeed  = await UserModel.find( {_id: {$nin: Array.from(myAllConnections) } } ).select(["firstName", "lastName", "skills", "gender", "age", "about", "pictureUrl"]).skip(skip).limit(limit);
